...
impl Msg {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        serde_json::to_vec(self).expect("Could not serialize into bytes!")
    }
    ...
    #[cfg_attr(feature = "wasm", wasm_bindgen(js_name = "toJson"))]
    pub fn to_json(&self) -> String {
        serde_json::to_string(&self).expect("Could not serialize Msg as JSON")
    }

    #[cfg_attr(feature = "wasm", wasm_bindgen(js_name = "fromJson"))]
    ...
    pub fn from_bytes(bytes: &[u8]) -> Msg {
        let msg: Msg =
            serde_json::from_slice(bytes).expect("Could not deserialize Msg from bytes!");
        msg
    }
}
